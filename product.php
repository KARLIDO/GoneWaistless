...
try {
    ...
    $images_query = "SELECT image_path FROM product_images WHERE product_id = :id";
    $stmt = $pdo->prepare($images_query);
    $stmt->bindParam(':id', $product_id, PDO::PARAM_INT);
    $stmt->execute();
    $images = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Fetch variants with quantity > 0
    $variants_query = "SELECT * FROM product_variants WHERE product_id = :id AND quantity > 0 ORDER BY color, size";
    $stmt = $pdo->prepare($variants_query);
    $stmt->bindParam(':id', $product_id, PDO::PARAM_INT);
    $stmt->execute();
    ...
    foreach ($variants as $variant) {
        ...
        $colors[$variant['color']]['sizes'][] = $variant['size'];
        
        // Check if any variant of this color has stock
        if ($variant['quantity'] > 0) {
            $colors[$variant['color']]['has_stock'] = true;
        }
    }
    
} catch (PDOException $e) {
    $error_message = "Unable to load product details at this time.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    $variant_id = 0;
    foreach ($variants as $variant) {
        if ($variant['color'] === $selected_color && $variant['size'] === $selected_size) {
            $valid_variant = true;
    ...
    if ($valid_variant) {
        ...
        foreach ($_SESSION['cart'] as &$item) {
            if ($item['product_id'] == $product_id && $item['color'] == $selected_color && $item['size'] == $selected_size) {
        ...
        $error_message = "Please select valid options.";
    }
}

// Opening hours code from products.php can be copied here
$currentDay = date('w');
$days = [
    0 => 'Sunday Closed',
...
<head>
    ...
    <title><?php echo isset($product['name']) ? htmlspecialchars($product['name']) : 'Product Not Found'; ?> | Your Store Name</title>
...
<body>
      <div id="loading-overlay">
        <div class="loading-content">
            ...
            <div class="loading-subtext">Preparing your product...</div>
...
<div class="top-bar">
        ...
        <div class="product-detail-container">
            <?php if (isset($product) && $product): ?>
                <div class="product-images">
                    <img src="<?php echo htmlspecialchars($product['main_image']); ?>" alt="<?php echo htmlspecialchars($product['name']); ?>" class="main-image" id="mainImage">
                ...
                <div class="product-info">
                    ...
                    <div class="product-price">
                        R<?php echo number_format($product['is_on_sale'] ? $product['sale_price'] : $product['price'], 2); ?>
                    ...
                    <form method="post" class="options-form">
                        <?php if (!empty($colors)): ?>
                            <div class="option-group">
                                ...
                                <select name="color" class="option-select" id="colorSelect" required>
                                    <option value="">Select Color</option>
                                    <?php foreach ($colors as $color => $colorData): ?>
                                        <option value="<?php echo htmlspecialchars($color); ?>" 
                                            <?php if ($colorData['has_stock'] === false) echo 'disabled'; ?>>
                                            <?php echo htmlspecialchars($color); ?>
                                            <?php if ($colorData['has_stock'] === false) echo '(Out of Stock)'; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="option-group">
                                ...
                                <select name="size" class="option-select" id="sizeSelect" required>
                                    <option value="">Select Size</option>
                                    <!-- This will be populated by JavaScript based on color selection -->
                                </select>
                            </div>
    ...
    <!-- Add this section right before the closing </section> tag in product.php -->
<?php if (isset($product) && $product): ?>
    <?php
    ...
    // Only show the section if there are related products
    if (!empty($related_products)): ?>
        <section class="related-products-section" style="padding: 50px 20px; background-color: #f9f9f9;">
            <div class="container" style="max-width: 1200px; margin: 0 auto;">
                <h2 class="section-title" style="text-align: center; color: #333; margin-bottom: 30px; font-size: 2rem; font-weight: 600;">
                    You May Also Like
                </h2>
                
                <div class="products-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 30px; padding: 20px 0;">
                    <?php foreach($related_products as $related): ?>
                        <?php
                        $price = $related['is_on_sale'] ? $related['sale_price'] : $related['price'];
                        $original_price = $related['is_on_sale'] ? '<span class="original-price" style="text-decoration: line-through; color: #999; font-size: 0.9rem; margin-left: 8px; font-weight: normal;">R' . number_format($related['price'], 2) . '</span>' : '';
                        ?>
                        
                        <div class="product-card" style="background: white; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1); transition: all 0.3s ease; position: relative;">
                            <a href="product.php?id=<?php echo $related['id']; ?>">
                                <div class="product-image" style="height: 300px; overflow: hidden; position: relative; display: flex; align-items: center; justify-content: center;">
                                    <img src="<?php echo htmlspecialchars($related['main_image']); ?>" alt="<?php echo htmlspecialchars($related['name']); ?>" style="width: auto; height: 100%; max-width: 100%; max-height: 100%; object-fit: contain; transition: transform 0.5s ease;">
                                    <?php if ($related['is_on_sale']): ?>
                                        <span class="sale-badge" style="position: absolute; top: 15px; right: 15px; background-color: #f557ab; color: white; padding: 5px 10px; border-radius: 20px; font-size: 12px; font-weight: bold;">SALE</span>
                                    <?php endif; ?>
                                </div>
                            </a>
                            <div class="product-info" style="padding: 20px; text-align: center;">
                                <h3 class="product-name" style="color: #333; font-size: 1.1rem; margin: 0 0 10px; font-weight: 600; height: 40px; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
                                    <?php echo htmlspecialchars($related['name']); ?>
                                </h3>
                                <div class="product-price" style="color: #f557ab; font-size: 1.2rem; font-weight: 700; margin-bottom: 15px;">
                                    R<?php echo number_format($price, 2); ?>
                                    <?php echo $original_price; ?>
                                </div>
                                <button class="add-to-cart" onclick="window.location.href='product.php?id=<?php echo htmlspecialchars($related['id']); ?>'" style="display: inline-block; padding: 10px 20px; background-color: #f557ab; color: white; border: none; border-radius: 30px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; width: 100%; max-width: 200px;">
...
<footer class="footer">
    ...
    <!-- Copy the WhatsApp button, chatbot, and footer from products.php -->
    ...
    <!-- Copy the JavaScript from products.php -->
    <script>
        ...
        // Add this JavaScript for the product page functionality
document.addEventListener('DOMContentLoaded', function() {
   
    
    // Size selector based on color
    ...
    if (colorSelect && sizeSelect) {
        ...
        colorSelect.addEventListener('change', function() {
            ...
            if (selectedColor) {
                ...
                const availableSizes = colors[selectedColor]['sizes'];
                
                // Filter variants for this color with quantity > 0
                const availableVariants = variants.filter(v => 
                    v.color === selectedColor && v.quantity > 0
                );
        ...
        // Trigger change event if color is preselected (after form submission)
        if (colorSelect.value) {
            colorSelect.dispatchEvent(new Event('change'));
            if (sizeSelect.querySelector(`option[value="${sizeSelect.dataset.preselected}"]`)) {
                sizeSelect.value = sizeSelect.dataset.preselected;
            }
        }
    }
